...
const { SHA1_BASE64, CERTICATE_DIGITAL, RSA_SHA256} = require('./util/signature')
...
async function sing(xml,password,p12,type = 01){
    ...
    if(type == 07)
        xmlFinal = xml.replace('</comprobanteRetencion>', xades_bes + '</comprobanteRetencion>')
    else if(type == 06)
        xmlFinal = xml.replace('</guiaRemision>', xades_bes + '</guiaRemision>')
    else
